...
The following table lists the configurable parameters of the AnalyticKit chart and their default values.
...
| env | list | `[]` | Environment variables to inject into every AnalyticKit deployment. |
| migrate.enabled | bool | `true` | Whether to install the AnalyticKit migrate job or not. |
| events.enabled | bool | `true` | Whether to install the AnalyticKit events stack or not. |
...
| web.enabled | bool | `true` | Whether to install the AnalyticKit web stack or not. |
...
| web.internalMetrics.capture | bool | `true` | Whether to capture information on operation of analytickit into analytickit, exposed in /instance/status page |
...
| plugins.enabled | bool | `true` | Whether to install the AnalyticKit plugin-server stack or not. This service handles data ingestion into ClickHouse, running apps and async jobs. To scale these components separately, instead use the individual component deployments, pluginsIngestion, pluginsAsync, pluginsJobs, and pluginsScheduler. |
...
| pluginsAsync.enabled | bool | `false` | Whether to install the AnalyticKit plugin-server async stack or not. If disabled (default), plugins service handles both ingestion and running of async tasks. Allows for separate scaling of this service. |
...
| pluginsIngestion.enabled | bool | `false` | Whether to install the AnalyticKit plugin-server ingestion capability as an individual workload. |
...
| pluginsExports.enabled | bool | `false` | Whether to install the AnalyticKit plugin-server exports capability as an individual workload. |
...
| pluginsJobs.enabled | bool | `false` | Whether to install the AnalyticKit plugin-server jobs capability as an individual workload. |
...
| pluginsScheduler.enabled | bool | `false` | Whether to install the AnalyticKit plugin-server scheduler capability as an individual workload. |
...
| email.user | string | `"***REMOVED-EXPOSED-AWS-KEY***"` | SMTP service user. |
...
| service.name | string | `"analytickit"` | AnalyticKit service name. |
| service.type | string | `"NodePort"` | AnalyticKit service type. |
...
| postgresql.nameOverride | string | `"analytickit-postgresql"` | Name override for PostgreSQL app. |
| postgresql.postgresqlDatabase | string | `"analytickit"` | PostgreSQL database name. |
...
| redis.nameOverride | string | `"analytickit-redis"` |  |
...
| kafka.nameOverride | string | `"analytickit-kafka"` |  |
...
| kafka.externalZookeeper.servers | list | `["analytickit-analytickit-zookeeper:2181"]` | List of external zookeeper servers to use. |
| externalKafka.brokers | list | `[]` |  |
| zookeeper.enabled | bool | `true` | Whether to deploy Zookeeper as part of this release. |
| zookeeper.nameOverride | string | `"analytickit-zookeeper"` |  |
...
| clickhouse.cluster | string | `"analytickit"` | Clickhouse cluster |
| clickhouse.database | string | `"analytickit"` | Clickhouse database |
...
| clickhouse.existingSecret | string | `""` | Clickhouse existing secret name that needs to be in the namespace where analytickit is deployed into. Will not use the above password value if set |
...
| clickhouse.image.tag | string | `"22.3.13.80"` | ClickHouse image tag. Note: AnalyticKit does not support all versions of ClickHouse. Please override the default only if you know what you are doing. |
...
| clickhouse.defaultSettings.default_database | string | `"analytickit"` |  |
...
| clickhouse.client.image.tag | string | `"22.3.13.80"` | ClickHouse image tag. Note: AnalyticKit does not support all versions of ClickHouse. Please override the default only if you know what you are doing. |
...
| clickhouse.backup.clickhouse_services | string | `"chi-analytickit-analytickit-0-0"` |  |
...
| externalClickhouse.database | string | `"analytickit"` | Database name for the external cluster |
...
| minio.defaultBuckets | string | `"analytickit"` | Comma, semi-colon or space separated list of buckets to create at initialization (only in standalone mode) |
...
| grafana.sidecar | object | `{"dashboards":{"enabled":true,"folderAnnotation":"grafana_folder","label":"grafana_dashboard","provider":{"foldersFromFilesStructure":true}}}` | Sidecar configuration to automagically pull the dashboards from the `charts/analytickit/grafana-dashboard` folder. See [official docs](https://github.com/grafana/helm-charts/blob/main/charts/grafana/README.md) for more info. |
| grafana.datasources | object | `{"datasources.yaml":{"apiVersion":1,"datasources":[{"access":"proxy","isDefault":true,"jsonData":{"timeInterval":"60s"},"name":"Prometheus","type":"prometheus","url":"http://analytickit-prometheus-server"},{"access":"proxy","isDefault":false,"name":"Loki","type":"loki","url":"http://analytickit-loki-read:3100"},{"access":"proxy","isDefault":false,"jsonData":{"implementation":"prometheus"},"name":"Alertmanager","type":"alertmanager","url":"http://analytickit-prometheus-alertmanager"}]}}` | Configure Grafana datasources. See [docs](http://docs.grafana.org/administration/provisioning/#datasources) for more info. |
...
| loki.fullnameOverride | string | `"analytickit-loki"` |  |
| loki.nameOverride | string | `"analytickit-loki"` |  |
...
| promtail.config.clients[0].url | string | `"http://analytickit-loki-write:3100/loki/api/v1/push"` |  |
...
| promtail.config.snippets.pipelineStages[2].match.selector | string | `"{app=\"analytickit\", container=~\"analytickit-web|analytickit-worker|analytickit-events\"}"` |  |
...
| prometheus-kafka-exporter.kafkaServer | list | `["analytickit-analytickit-kafka:9092"]` | Specify the target Kafka brokers to monitor. |
...
| prometheus-postgres-exporter.config | object | `{"datasource":{"host":"analytickit-analytickit-postgresql","passwordSecret":{"key":"postgresql-password","name":"analytickit-analytickit-postgresql"},"user":"postgres"}}` | Configuration options. |
...
Dependent charts can also have values overwritten. For more info see our [docs](https://analytickit.com/docs/self-host/deploy/configuration).
