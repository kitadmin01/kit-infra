...
{{- define "snippet.error-on-invalid-values" }}
  ...
  {{- else if .Values.certManager }}
    {{- required (printf (include "snippet.error-on-invalid-values-template" .)
      ...
      "https://analytickit.com/docs/self-host/deploy/upgrade-notes#upgrading-from-3xx"
    ) nil -}}

  {{- else if .Values.beat }}
    {{- required (printf (include "snippet.error-on-invalid-values-template" .)
      "beat deployment has been removed"
      "https://analytickit.com/docs/self-host/deploy/upgrade-notes#upgrading-from-7xx"
    ) nil -}}

  {{- else if .Values.clickhouseOperator }}
    {{- required (printf (include "snippet.error-on-invalid-values-template" .)
      "clickhouseOperator values are no longer valid"
      "https://analytickit.com/docs/self-host/deploy/upgrade-notes#upgrading-from-9xx"
    ) nil -}}
  ...
  {{- else if or .Values.redis.port .Values.redis.host .Values.redis.password }}
    {{- required (printf (include "snippet.error-on-invalid-values-template" .)
      ...
      "https://analytickit.com/docs/self-host/deploy/upgrade-notes#upgrading-from-11xx"
    ) nil -}}

  {{- else if .Values.clickhouse.host }}
    {{- required (printf (include "snippet.error-on-invalid-values-template" .)
      "clickhouse.host has been moved to externalClickhouse.host"
      "https://analytickit.com/docs/self-host/deploy/upgrade-notes#upgrading-from-13xx"
    ) nil -}}

  {{- else if .Values.clickhouse.replication }}
    {{- required (printf (include "snippet.error-on-invalid-values-template" .)
      "clickhouse.replication has been removed"
      "https://analytickit.com/docs/self-host/deploy/upgrade-notes#upgrading-from-12xx"
    ) nil -}}
