...
kind: Deployment
metadata:
  name: {{ template "analytickit.fullname" .root }}-{{ .name }}
...
spec:
  selector:
    matchLabels:
        app: {{ template "analytickit.fullname" .root }}
  ...
  template:
    metadata:
      ...
      labels:
        ...
        {{- end }}
    spec:
      serviceAccountName: {{ template "analytickit.serviceAccountName" .root }}
      {{- if .params.affinity }}
      affinity:
      ...
      - name: {{ .root.Chart.Name }}-{{ .name }}
        image: {{ template "analytickit.image.fullPath" .root }}
        ...
        {{- include "snippet.analytickit-env" .root | indent 8 }}
