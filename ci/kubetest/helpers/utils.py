...
def helm_install(HELM_INSTALL_CMD):
    log.debug("🔄 Deploying AnalyticKit...")
...
def wait_for_pods_to_be_ready(kube, labels=None, expected_count=None, namespace=NAMESPACE):
    ...
    while time.time() < start + timeout:
        ...
        for pod in pods.values():
            ...
            if pod.obj.metadata.labels.get("app") == "analytickit" and not name.startswith("analytickit-pgbouncer"):
...
def is_analytickit_healthy(kube):
    test_if_analytickit_deployments_are_healthy(kube)


def test_if_analytickit_deployments_are_healthy(kube):
